...
from ableton.v2.base import EventObject, find_if, listens, listens_group, listenable_property, liveobj_valid, old_hasattr, clamp
import Live
import logging
...
class DeviceParameterBank(EventObject):
    ...
    def __init__(self, size = None, device = None, banking_info = None, *a, **k):
        ...
        if hasattr(device, 'visible_macro_count'):
            self._size = self._device.visible_macro_count
            device.add_visible_macro_count_listener(self._on_visible_macro_count_changed)

        self._update_parameters()

    def disconnect(self):
        if hasattr(self._device, 'visible_macro_count'):
            self._device.remove_visible_macro_count_listener(self._on_visible_macro_count_changed)
        super(DeviceParameterBank, self).disconnect()

    def _on_visible_macro_count_changed(self):
        self._size = self._device.visible_macro_count
        self._update_parameters()
    ...
    def _collect_parameters(self):
        ...
        result = list([(None, None)] * self._size)

        transpose = isinstance(self._device, Live.PluginDevice.PluginDevice)
        for i in range(min(len(params), self._size)):
            dst_index = PARAMETER_TRANSPOSE[i] if transpose else i
            result[dst_index] = (params[i], None)
...
class DescribedDeviceParameterBank(DeviceParameterBank):
    ...
    def _current_parameter_slots(self):
        if self.index >= len(self._definition):
            return tuple()
...
def create_device_bank(device, banking_info):
    ...
    if liveobj_valid(device):
        ...
        bank = bank_class(device=device, size=BANK_SIZE, banking_info=banking_info)
        #logger.info(u'Create bank for: %s (%s)', type(bank).__name__, type(bank_class).__name__)
    return bank
